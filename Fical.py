...
def ergebnisse_ausgeben():
    ...
    def type_comparission(index=0):
        ...
        if index < len(text_better_worse):
            last_time_label.config(text=last_time_label.cget("text") + text_better_worse[index])
...
def ai_chat():
    ...
    ai_button1 = tk.Button(root, text="Ich möchte einen Test Durchlauf mit zufälligen Zahlen.", command=ai_answers.testdurchlauf)
...
def ai_assistant():
    for w in root.winfo_children():
        w.destroy()
    file_path = get_file_dir("ai_advertise.png")
    pic = Image.open(file_path).resize((350, 350))
    ai_advertise_image = ImageTk.PhotoImage(pic)
    ...
    pic = Image.open(file_path).resize((150, 150))
    start_chat_image = ImageTk.PhotoImage(pic)

    start_chat = tk.Button(root, image=start_chat_image, highlightthickness=0, borderwidth=0, cursor="hand2", command=ai_chat)
...
def main_menu():
    ...
    bizhar_ai_button = tk.Button(root, image=bizhar_ai_image, borderwidth=0, highlightthickness=0, cursor="hand2", command=ai_assistant)
...
def start_loading():
    ...
    control_popup_file = "popupcontrol.txt"
    file_path = os.path.join(current_dir, control_popup_file)
    if os.path.isfile(file_path):
        remove_labels()
    else: 
        show_premium_popup()
